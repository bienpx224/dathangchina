                ...
                <table class="table table-hover list-sp" style="max-width: 80%; border: 1px;">
                    ...
                    @foreach ($sanpham as $sp)
                        ...
                        @elseif ( $sp->status == 1)
                            <tr class="info">
                                @endif
                                <form action="{!! route('adminUpdateProductUser') !!}" method="post" enctype="multipart/form-data">
                                    <td><div title="{{$sp->title}}"> {{$sp->title}}</div></td>
                                    <td> <a title="{{$sp->link}}" target="_blank" href="{{$sp->link}}">{{$sp->link}}</a></td>
                                    <td> <img style="max-height: 120px; max-width: 100px; overflow: hidden;" class="responsive" src="{{$sp->image}}"/></td>
                                    ...
                                    <td>
                                        ...
                                        @if($sp->status == 1 && $sp->state == 1)
                                            <form  action="{!! route('adminOkProductUser') !!}" method="post" enctype="multipart/form-data">
                                                <input type="hidden" name="_token" value="{{csrf_token()}}">
                                                <input type="hidden" name="id_sp" value="{{$sp->id}}">
                                        ...
                                        @endif
                                        @if($sp->status == 1 && $sp->state == 1)
                                            <form  action="{!! route('adminOutOfStockProductUser') !!}" method="post" enctype="multipart/form-data">
                                                <input type="hidden" name="_token" value="{{csrf_token()}}">
                                                <input type="hidden" name="id_sp" value="{{$sp->id}}">
